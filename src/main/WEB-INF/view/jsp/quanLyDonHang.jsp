	...
	$(document)
			.ready(
					function() {
						...
						$('.dataTables_filter')
								.append(
										"<br><br><a 	href='<c:url value="./DonHang"></c:url>' class='ml-2 btn btn-warning'>Làm mới dữ liệu bảng</a><br><br>");
					});
</script>
</head>
<body>
					...
					<div class="product-block">
						...
						<table id="table" class="table table-striped table-bordered"
							...
							<thead>
								...
								<!-- 	<th width="40">Xác nhận</th>
									<th width="40">Từ chối</th> -->
									<th width="40">Xem chi tiết đơn hàng</th>

								</tr>
							</thead>
							<tbody>
								<c:forEach var="hd" items="${donFalse}">
									<tr>
										<td><a
											href='<c:url value="CTDonHang?id=${hd.id}"></c:url>'
											class="mau1">${hd.id}</a></td>
										<td><spring:eval expression="hd.ngayLap" /></td>
										<td>${hd.kh.hoTen }</td>						
										<td width="120"><fmt:formatNumber value="${hd.tongTien}" pattern="#,### đ"></fmt:formatNumber></td>
									<%-- 	<td width="40"><a
											href='<c:url value="XNDonHang?id=${hd.id}"></c:url>'
											class="btn btn-success"
											onclick="return confirm('Bạn có chắc chắn xác nhận đơn hàng này không?')">Xác
												nhận</a></td>
										<td><a
											href='<c:url value="TuChoiDonHang?id=${hd.id}"></c:url>'
											class="btn btn-danger"
											onclick="return confirm('Bạn có chắc chắn từ chối nhận đơn hàng này không?')">Từ
												chối</a></td> --%>
										<td><a
											href='<c:url value="CTDonHang?id=${hd.id}"></c:url>'
											class="btn btn-primary">Xem chi tiết đơn hàng</a></td>
						...
						<table id="tablePerson" class="table table-striped table-bordered"
							style="width: 100%">
							<thead>
								<tr>
									<th>Mã đơn hàng</th>
							...
							<tbody>
								<c:forEach var="hd" items="${donTrue}">
									<tr>
										<td><a
											href='<c:url value="CTDonHang?id=${hd.id}"></c:url>'
											class="mau1">${hd.id}</a></td>
